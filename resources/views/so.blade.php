            ...
            <div class="card-body">
              <div>
                <form action="/sales" method="post">
                    @csrf
                    <div class="mb-3">
                        <select class="form-select" aria-label="Default select example" name="nama_customer" id="nama_customer">
                            <option selected>--- Select Customer ---</option>
                            @foreach ($customer as $item)
                                <option value="{{$item->nama_customer}}" name="nama_customer">{{$item->nama_customer}}</option>
                            @endforeach
                        </select>
                    </div>
              ...
              <div>
                <a href="/reportSo" class="btn btn-danger" style="text-decoration: none; color:white">Cancel</a>
            </div>
            </div>
        </div>
    </div>
